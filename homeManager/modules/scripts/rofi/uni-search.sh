...
if [[ -n "$URLflag" ]]; then
	...
	if [[ -n "$user_input" ]]; then
		...
		zen-beta --new-tab "https://www.google.com/search?q=${user_input}"
	...
	if [[ -n "$user_input" ]]; then
		...
		zen-beta --new-tab "https://chat.openai.com/?q=${user_input}"
		# grim -g "$(slurp -d)" - | aichat -f | rofi -dmenu -p ""
	...
	if [[ -n "$user_input" ]]; then
		...
		zen-beta --new-tab "https://9animetv.to/search?keyword=${user_input}"
	...
	if [[ -n "$user_input" ]]; then
		...
		zen-beta --private-window "https://www.google.com/search?q=${user_input}"
	...
	if [[ -n "$user_input" ]]; then
		...
		zen-beta --new-tab "https://search.nixos.org/packages?channel=unstable&from=0&size=50&sort=relevance&type=packages&query=${user_input}"
